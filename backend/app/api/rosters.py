...
def get_roster(exam_id: int, session: Session = Depends(get_db)):
...
async def upload_roster(
    ...
    if suffix == "pdf":
        ...
        if len(data) > settings.max_upload_bytes:
            raise HTTPException(status_code=413, detail="Uploaded roster PDF is too large")
    ...
    if suffix in _TABLE_SOURCES:
        data = await file.read()
        if len(data) > settings.max_upload_bytes:
            raise HTTPException(status_code=413, detail="Uploaded roster file is too large")
        try:
...
def confirm(exam_id: int, session: Session = Depends(get_db)):
    _load_exam_or_404(session, exam_id)
    try:
...
def delete_roster(exam_id: int, session: Session = Depends(get_db)):
    _load_exam_or_404(session, exam_id)
    try:
